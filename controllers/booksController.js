...
exports.borrowBook = async (req, res, next) => {
  try {
    ...
    if (!borrower) return res.status(400).json({ success:false, message: 'Borrower required' });
    const book = await Book.findById(req.params.id);
    if (!book) return res.status(404).json({ success:false, message:'Not found' });
    if (!book.available) return res.status(400).json({ success:false, message:'Already borrowed' });
    book.available = false;
    book.borrower = borrower;
    book.borrowedAt = new Date();
    await book.save();
    res.json({ success:true, message:'Borrowed', data: book });
  } catch (err) { next(err); }
};
...
exports.returnBook = async (req, res, next) => {
  try {
    const book = await Book.findById(req.params.id);
    if (!book) return res.status(404).json({ success:false, message:'Not found' });
    if (book.available) return res.status(400).json({ success:false, message:'Book is not borrowed' });
    book.available = true;
    book.borrower = undefined;
    book.borrowedAt = undefined;
    await book.save();
    res.json({ success:true, message:'Returned', data: book });
  } catch (err) { next(err); }
};